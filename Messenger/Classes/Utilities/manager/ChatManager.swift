...
class ChatManager: NSObject {
	...
	@objc private func initObservers() {
		...
		if (AuthUser.userId() != "") {
			...
			if (tokenActions == nil)	{ loadActions() }
	...
	@objc private func stopObservers() {
		...
		tokenActions?.invalidate();		tokenActions = nil
	...
	private func update(with action: Action) {
		...
		var temp: [String: Any] = [:]
		temp["objectId"] = action.chatId

		if (action.userId == AuthUser.userId()) {
			temp["lastRead"]	= action.lastRead
			temp["mutedUntil"]	= action.mutedUntil
			temp["unreadCount"]	= unreadCount(action.chatId, action.lastRead)
		}

		if (action.userId != AuthUser.userId()) {
			let predicate = NSPredicate(format: "chatId == %@ AND userId != %@ AND typing == YES", action.chatId, AuthUser.userId())
			temp["typing"] = (realm.objects(Action.self).filter(predicate).count != 0)
		}
