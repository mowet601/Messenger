...
class RCMessageVideoCell: RCMessageCell {
	...
	override func bindData(_ messagesView: RCMessagesView, at indexPath: IndexPath) {
		...
		viewBubble.backgroundColor = rcmessage.incoming ? RCDefaults.videoBubbleColorIncoming : RCDefaults.videoBubbleColorOutgoing
		...
		if (imageViewPlay == nil) {
			imageViewPlay = UIImageView(image: RCDefaults.videoImagePlay)
		...
		if (imageViewManual == nil) {
			imageViewManual = UIImageView(image: RCDefaults.videoImageManual)
